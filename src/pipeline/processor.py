...
from src.model.insight_analysis import lineage_insights, lineage_analysis, grid_insights, bumapper, workload_analysis
...
class Pipeline:
    ...
    def __init__(self, params,config):
        ...
        self.output_path = os.path.join(data_dir, self.config_data['asia_asset']['output_path'])
        self.file_adapter = Reader(self.input_path, self.config_data)

    def run(self):
        # load the environment config
        
        Error_Path = os.path.join(self.input_path,self.config_data['asia_asset']['Error_Path'])
        path_map = os.path.join(self.input_path,self.config_data['asia_asset']['path_map'])
    ...
    def meta_retriver(self):
        ...
        if type(self.config_data["asia_asset"]["log_location"]) == list:
            ...
            logger.info("Collecting event files for {env} environment.".format(env=self.env))
        elif type(self.config_data["asia_asset"]["log_location"]) == str:
            all_files = glob.glob(os.path.join(self.input_path,self.config_data["asia_asset"]["log_location"]))
            #event_files,log_files = get_event_and_logs(all_files)
            logger.info("Collecting event files for {env} environment.".format(env = self.env))
        ...
        while number_of_files < len(all_files):
            event_files, log_files = get_event_and_logs(all_files[number_of_files:number_of_files+batch_size])
        ...
        lsf_output = pd.concat([lsf_output,lsf_sas(lsf_output,self.config_data['asia_asset']['code_location'])])
        logger.info("Running adsreader reader for {env} environment".format(env=self.env))
    ...
    def sas_parser(self,lsf_output):
        ...
        while number_of_files < lsf_output[lsf_output.columns[0]].count():
            ...
            sas_output = self.file_adapter.sas_reader(lsf_output[number_of_files:number_of_files+batch_size], self.config_data)
            # calling egp_reader
            #logger.info("Running egpreader reader for {env} environment".format(env=self.env))
            egp_output = self.file_adapter.egp_reader(lsf_output[number_of_files:number_of_files+batch_size], self.config_data)
            # #calling sas_output
    ...
    def analyzer(self,final_output,sas_bus_unit):
        ...
        while number_of_files < final_output[final_output.columns[0]].count():
            ...
            temp_final_output = final_output[number_of_files:number_of_files+ref_batch_size]
            logger.info("Running libname reader reader for {env} environment".format(env=self.env))
            libname_output, table_details = self.file_adapter.libname_reader(temp_final_output, self.InOutMap)
            ...
            if libname_output is None or libname_output.empty:
                ...
                ref_batch_size += 5
                continue
            logger.info("Running grid search for {env} environment".format(env=self.env))
            ...
            logger.info("Running BU Mapper operation on sas_output for {env} environment".format(env=self.env))
            merge_output_bumapped = bumapper(self.input_path, temp_final_output, 'merge_output', sas_bus_unit)
            # #calling bumapper for libname output
            logger.info("Running BU Mapper operation on libname_output for {env} environment".format(env=self.env))
            libname_output_bumapped = bumapper(self.input_path, libname_output, 'libname_output', sas_bus_unit)
            # #calling bumapper for grid output
            logger.info("Running BU Mapper operation on grid_search_output for {env} environment".format(env=self.env))
            grid_output_bumapped = bumapper(self.input_path, grid_search_output, 'distributed_grid', sas_bus_unit)
            logger.info("Running workload insight for {env} environment".format(env=self.env))
            workload = workload_insights(merge_output_bumapped)
            # #calling lineage_insights
            logger.info("Running lineage insight for {env} environment".format(env=self.env))
            lineage = lineage_insights(libname_output_bumapped)
            # #calling grid_insights
            logger.info("Running grid insight for {env} environment".format(env=self.env))
            grid = grid_insights(grid_output_bumapped)
            # #calling workload_analysis
            logger.info("Running workload analysis for {env} environment".format(env=self.env))
            workload_overview = workload_analysis(workload, grid, self.config_data)
            # #calling lineage_analysis
            logger.info("Running lineage analysis for {env} environment".format(env=self.env))
            lineage_overview = lineage_analysis(workload_overview, lineage)
            # #calling artifact_90
            logger.info("Running artifact 90 for {env} environment".format(env=self.env))
            workload_dnd, lineage_dnd = artifact_90(self.input_path, workload_overview, lineage_overview)
            # #calling bom_usage
            logger.info("Generating BOM(Bill of material) Report for {env} environment".format(env=self.env))
            bom_usage_dnd, workload_dnd_usg, lineage_dnd_usg = bom_usage(workload_dnd, lineage_dnd, self.input_path)
            ...
            logger.info("Writing Workload Analysis Report for {env} environment".format(env=self.env))
            workload_dnd_usg.to_csv(self.output_path +"/workload_dnd_{0}.csv".format(batch_no), index=None)
            logger.info("Writing Lineage Overview Report for {env} environment".format(env=self.env))
            lineage_dnd_usg.to_csv(self.output_path + '/lineage_dnd_{0}.csv'.format(batch_no), index=None)
            logger.info("Writing Bill Of Material usage Report for {env} environment".format(env=self.env))
            bom_usage_dnd.to_csv(self.output_path + '/bom_usage_dnd_{0}.csv'.format(batch_no), index=None)
            logger.info("Writing Table Detail Report for {env} environment".format(env=self.env))
            table_details.to_csv(self.output_path + '/table_details_{0}.csv'.format(batch_no), index=None)
